#!/usr/bin/env python3
"""Train the 23-class lateral-spine YOLO11 Pose model on AutoDL."""
...
def validate_dataset(data_yaml: Path) -> dict[str, Any]:
    """Validate dataset structure and every YOLO Pose label before training."""
    data_yaml = data_yaml.resolve()
    if not data_yaml.is_file():
        raise FileNotFoundError(f"Dataset YAML not found: {data_yaml}")
    yaml_text = data_yaml.read_text(encoding="utf-8")
    if "nc: 23" not in yaml_text or "kpt_shape: [4, 3]" not in yaml_text:
        raise ValueError("Dataset YAML must declare nc: 23 and kpt_shape: [4, 3]")
    ...
    for split in ("train", "val", "test"):
        ...
        for path in labels.values():
            for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
                ...
                if [values[index] for index in (6, 9, 12, 15)] != [2.0, 2.0, 2.0, 2.0]:
                    raise ValueError(f"{location}: expected four visible keypoints")
                all_ids.add(class_id)
                split_objects += 1
        report["splits"][split] = {"images": len(images), "objects": split_objects}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data",
        type=Path,
        default=project_root / "datasets/yolo_lateral_20260903_23cls/data.yaml",
    )
    parser.add_argument(
        "--model",
        default="yolo11m-pose.pt",
        help="YOLO pose checkpoint or model name; the old corner_model.pt can be used for transfer learning",
    )
    ...
    parser.add_argument("--name", default="yolo11m_lateral_23cls")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Validate and print settings without importing Ultralytics")
