...
## Goal
完成第二批23类侧面脊柱数据集和三关键点骨盆数据集；现把第一批补标任务收紧为只补C2-C6，排除原始C7-L5不完整、C2-C6候选不完整或C2触边的样本，并在E盘生成新的严格人工复核可视化包。
...
- [ ] 统计原始C7-L5完整性，不接受模型补齐这18类
- [ ] 统计仅C2-C6候选完整性和C2边界安全距离
- [ ] 固定建议纳入、建议排除及排除原因优先级
- **Status:** in_progress

### Phase 36: 严格C2-C6复核包实现
- [ ] 只把C2-C6模型候选叠加到原始JSON，不显示其他伪标签
- [ ] 在图像、CSV与HTML中明确纳入状态和排除原因
- [ ] 增加C2触边判定、过滤和回归测试
- **Status:** pending
