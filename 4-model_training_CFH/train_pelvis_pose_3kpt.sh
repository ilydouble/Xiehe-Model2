...
show_help() {
    cat <<'EOF'
CFH + S1 三关键点 YOLO Pose 训练脚本

用法:
    ./train_pelvis_pose_3kpt.sh [预设] [选项]
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --quick)
            ...
            IMGSZ=640
            BATCH=8
            NAME="yolo11n_pelvis_3kpt_quick"
            shift
            ;;
        --standard)
            MODEL="yolo11m-pose.pt"
...
if ! ${DRY_RUN} && ! python3 -c "import ultralytics" 2>/dev/null; then
    ...
    print_info "请运行: pip install -r 4-model_training_CFH/requirements.txt"
    exit 1
fi
print_success "Python环境检查通过"

COMMAND=(
    python3 "${SCRIPT_DIR}/train_pelvis_pose_3kpt.py"
    --data "${DATA}"
    --model "${MODEL}"
    --epochs "${EPOCHS}"
    --imgsz "${IMGSZ}"
